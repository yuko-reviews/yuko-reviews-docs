...
2. **Scheduled reconciliation** — a daily cron (`yuko_reactivate_disabled_webhooks`) re-enables any webhooks WooCommerce disabled and backfills missed events.
...
<AccordionGroup>
  ...
  <Accordion title="Webhook disabled by WooCommerce" icon="plug-circle-xmark">
    ...
    Fix automatically: wait for the daily `yuko_reactivate_disabled_webhooks` cron (runs at ~midnight UTC).

    Fix now: **WordPress Admin → WooCommerce → Settings → Advanced → Webhooks** — find Yuko webhooks and set status to **Active**.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Order older than 90 days" icon="calendar-xmark">
    Historical orders are not imported during initial sync — only orders from the last 90 days. If you need to review an older order, use [Manual review requests](/collect-reviews/manual-review-requests).
  </Accordion>
