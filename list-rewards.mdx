...
```bash cURL
curl --request GET \
  --url "https://api.yukoapp.com/v1/external/rewards?reward_type=fixed_discount" \
...
const response = await fetch(
  `https://api.yukoapp.com/v1/external/rewards?${params}`,
  {
    method: "GET",
    headers: {
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.yukoapp.com/v1/external/rewards?{$query}");
