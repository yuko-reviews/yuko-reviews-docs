...
```bash cURL
curl --request GET \
  --url "https://api.yukoapp.com/v1/external/earning-rules?campaign_type=purchase" \
...
const response = await fetch(
  `https://api.yukoapp.com/v1/external/earning-rules?${params}`,
  {
    method: "GET",
    headers: {
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.yukoapp.com/v1/external/earning-rules?{$query}");
