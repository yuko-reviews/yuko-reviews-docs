...
**GET /external/customers/\{customerId\}/activities**
...
The UUID of the customer whose activities to retrieve.
...
```bash cURL
curl --request GET \
  --url "https://api.yukoapp.com/v1/external/customers/63a2c89a-9936-4bc0-9d5f-93db46110b76/activities?per_page=20" \
...
const response = await fetch(
  `https://api.yukoapp.com/v1/external/customers/${customerId}/activities?${params}`,
  {
    method: "GET",
    headers: {
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.yukoapp.com/v1/external/customers/{$customerId}/activities?{$query}");
