...
```bash cURL
curl --request GET \
  --url "https://api.yukoapp.com/v1/external/customers?per_page=20&page=1" \
...
const response = await fetch(
  `https://api.yukoapp.com/v1/external/customers?${params}`,
  {
    method: "GET",
    headers: {
...
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://api.yukoapp.com/v1/external/customers?{$query}");
